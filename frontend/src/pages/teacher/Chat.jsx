        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-white shadow-sm bg-emerald-100">
                {partner?.profilePic ? (
            ...
            return (
              ...
              >
                <div className={`max-w-[85%] md:max-w-[70%] px-4 py-3 rounded-2xl shadow-sm relative group ${
                  ...
                  <div className={`flex items-center gap-1 mt-1.5 ${isMe ? 'justify-end text-emerald-100' : 'justify-start text-slate-400'}`}>
                    <span className="text-[9px] font-bold uppercase">
                      {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                    {isMe && <CheckCheck size={12} className={msg.seen ? "text-emerald-200" : "opacity-50"} />}
                  </div>
                </div>
              </div>
            );
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2 items-center">
            <div className="flex-1 relative flex items-center">
              <input
                ...
                className="w-full bg-slate-100 border-none rounded-2xl px-5 py-3.5 text-sm focus:ring-2 focus:ring-emerald-500/20 placeholder:text-slate-400"
              />
            </div>
            <button
              type="submit"
              disabled={!newMessage.trim()}
              className="bg-emerald-600 text-white p-3.5 rounded-2xl shadow-lg shadow-emerald-500/30 hover:bg-emerald-700 active:scale-95 disabled:opacity-50 disabled:active:scale-100 transition-all"
            >
              <Send size={20} />
            </button>
          </form>
